...
export default function BracketView({ games, bracket, onPick }: BracketViewProps) {
  ...
  function StageColumn({ stage, label }: { stage: string; label: string }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", minWidth: colWidth, maxWidth: colWidth }}>
        <div style={{ textAlign: "center", padding: "8px 4px", marginBottom: 8, borderBottom: `1px solid ${CYAN}22` }}>
            ...
            return (
              <div key={g.gameId} style={{
                ...
                <div style={{ textAlign: "center" }}>
                  {started && !pick && <span style={{ fontSize: 9, color: "#444" }}>⏸ Locked</span>}
                  {!started && !pick && <span style={{ fontSize: 9, color: "#333" }}>Pick the winner</span>}
                  {pick && <span style={{ fontSize: 9, color: NEON, textShadow: `0 0 6px ${NEON}` }}>🎯 {pick}</span>}
                </div>
              </div>
            );
          })}
  ...
  const finalGames = stageGames("FINAL");
  const finalGame = finalGames.length > 0 ? finalGames[0] : null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      <div ref={scrollRef} style={{ overflowX: "auto", overflowY: "hidden", paddingBottom: 12, WebkitOverflowScrolling: "touch" }}>
        <div style={{ display: "flex", gap: 12, minWidth: "fit-content", padding: "0 4px", alignItems: "flex-start" }}>
          {stages.map((s) => <StageColumn key={s.key} stage={s.key} label={s.label} />)}
        </div>
      </div>
